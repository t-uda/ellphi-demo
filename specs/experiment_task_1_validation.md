...
## 1. Environment & Setup
- **Target**: `ellphi` library (Python).
- **Tools**: `uv` environment, `matplotlib` (LaTeX mode), `numpy`.
...
- **Procedure**:
    1. **Analytical Gradient**: Compute $\nabla_p \mu$ and $\nabla_q \mu$ using `ellphi.grad.tangency_grad` (or the legacy `ellphi.differentiable_solver.solve_mu_gradients`).
...
## 5. Status
**Done.** Implemented in `notebooks/task_1_validation/task_1_validation.py` (marimo notebook).
